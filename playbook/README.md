# Defra AI Legacy Modernisation Playbook
Version 1.0.0

## Purpose of this Playbook

This playbook provides guidance on how to modernise legacy systems using AI technologies. It is designed for internal use within Defra and is updated regularly with new practices and lessons learnt.

> **Note:** All examples provided in this playbook are **illustrative** and intended to show **contextual possibilities only**.  
> They do **not** represent confirmation of existing systems or projects currently in use by Defra.
> Any examples of potential AI tools and framework use (e.g. LangChain, Azure AI Studio, Semantic Kernel, OpenAI Assistants) are illustrative only. **Always consult the Defra AI Tools Authority or your technical governance lead** before selecting or implementing any AI technology to ensure alignment with internal standards, approved platforms, and procurement policy.

## Table of Contents
1. [Welcome](.)
2. [Responsible Practices](./pages/responsible-practices/)
   - [Ethics](./pages/responsible-practices/ethics)
   - [Sustainability](./pages/responsible-practices/sustainability)
3. [Getting Started](./pages/getting-started/)
   - [Four Pillars](./pages/getting-started/four-pillars)
   - [Workflow](./pages/getting-started/workflow)
4. [Discovery and Analysis](./pages/discovery-analysis/)
   - [Documentation Gathering](./pages/discovery-analysis/documentation-gathering)
   - [Requirements Building](./pages/discovery-analysis/requirements-building)
5. [Pathways](./pages/pathways/)
   - [MS Access](./pages/pathways/ms-access/)
     - [SSMA Migration](./pages/pathways/ms-access/database/01-ssma-migration)
     - [SQL Refactoring](./pages/pathways/ms-access/database/02-sql-refactoring)
     - [Document UI](./pages/pathways/ms-access/frontend/01-create-ui-documentation)
   - [SQL Server](./pages/pathways/sql-server/)
     - [Schema Extraction](./pages/pathways/sql-server/database/01-schema-extraction)
     - [Changelog Refactoring](./pages/pathways/sql-server/database/02-changelog-refactoring)
     - [MongoDB Decision Point](./pages/pathways/sql-server/database/03-mongodb-decision-point)
     - [MongoDB Migration](./pages/pathways/sql-server/database/04-mongodb-migration)
     - [OpenAPI Specification](./pages/pathways/sql-server/api/openapi-specification)
6. [Lessons Learnt](./pages/lessons-learnt/lessons-learnt)
7. Appendix
   - [Defra Approved Tools](./pages/appendix/defra-approved-tools)
   - [Prompt Library](./pages/appendix/prompt-library/)

## Overview

Defra manages environmental protection, food production and rural development across the UK. The department works through multiple sub-departments and arms-length bodies all of which have legacy systems that can benefit from modernisation.

---

[Next: Responsible Practices →](pages/responsible-practices/.)
